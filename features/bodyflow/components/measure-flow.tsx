...
export function MeasureFlow({ days, dense }: { days: BodyflowDay[]; dense: boolean }) {
  const series = SERIES.map((s) => {
    const raw: Maybe[] = days.map((d) => d[s.key]);
    return {
      ...s,
      raw,
  ...
  return (
    <div>
      <p className="mb-2 text-sm font-semibold text-[var(--text1)]">Mål &amp; vekt</p>
          ...
          <svg viewBox={`0 0 ${W} ${H}`} className="h-28 w-full" preserveAspectRatio="none">
            {series.map((s) => {
              ...
              return segments.map((seg, si) => (
                <g key={`${s.key}-${si}`}>
                  {seg.length === 1 ? (
                    <circle cx={seg[0].x} cy={seg[0].y} r={3} fill={s.color} />
                  ) : (
                    <polyline
